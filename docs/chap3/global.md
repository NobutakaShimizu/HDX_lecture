...
> $$
>
> で定め, それ以下の次元の定常分布$\pi^\sigma_j$は[定義2.2.1](#def:down_walk_and_stationary_distribution)によって定める.
...
[定義2.2.1](#def:down_walk_and_stationary_distribution)では次元$i$の面から次元$i-1$に遷移する下降ウォークを与えた.
...
>
> [定義2.2.1](#def:down_walk_and_stationary_distribution), [定義2.2.3](#def:up walk)と同じ設定を考える.
> それぞれ
>
> $$
    \begin{align*}
...
> [定義2.2.1](#def:down_walk_and_stationary_distribution), [定義2.2.3](#def:up walk)の呼称を採用している.
