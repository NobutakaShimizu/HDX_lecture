...
ある面$\tau\in X(i+1)$から分布$\Pdown_{i+1}(\tau,\cdot)$に従ってランダムに選ばれた面$\sigma$に遷移する過程を**下降ウォーク (down walk)**と呼ぶ.
...
<div id="rem:link_of_u" markdown="1">
{: .remark }
...
任意の$f\in\ispace{X(i)}$に対し、

$$
\begin{align*}
\iprod{X(i)}{f,\PDU_i f} = \iprod{X(i)}{f,\Pdown_i \Pup_{i-1} f} = \iprod{X(i-1)}{\Pup_{i-1}f,\Pup_{i-1}f}\ge 0.
\end{align*}
$$
